...
denseInput = open('cboxes2.json')
data = json.load(denseInput)
outputData = {}
outputData['predicates'] = []
outputData['boxes'] = []
index = 0
for caption in data['results'][0]['captions']:
	attribute1 = 'none'
	object1 = 'none'
	relation = 'none'
	attribute2 = 'none'
	...
	predicateText = "property(" + str(relation) + ", attribute(" + str(attribute1) + ", " + str(object1) + "), attribute(" + str(attribute2) + ", " + str(object2) + "), box" + str(data['results'][0]['captionBoxes'][index][0]) + ", box" + str(data['results'][0]['captionBoxes'][index][1]) + ")"
	outputData['predicates'].append(predicateText)
	index += 1
index = 0
for box in data['results'][0]['boxes']:
	boxText = "property(box" + str(index) + ", " + str(data['results'][0]['boxes'][index][0]) + ", " + str(data['results'][0]['boxes'][index][1]) + ", " + str(data['results'][0]['boxes'][index][2]) + ", " + str(data['results'][0]['boxes'][index][3]) + ")"
	outputData['boxes'].append(boxText)
	index += 1
denseInput.close()
with open('predicates2.json', 'w') as outfile:
