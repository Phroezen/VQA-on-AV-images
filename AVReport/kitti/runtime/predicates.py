...
for caption in data['results'][0]['captions']:
	...
	predicateText = "property(" + str(relation) + ", attribute(" + str(attribute1) + ", " + str(object1) + "), attribute(" + str(attribute2) + ", " + str(object2) + "), box" + str(data['results'][1]['captionBoxes'][index][0]) + ", box" + str(data['results'][1]['captionBoxes'][index][1]) + ")"
	outputData['predicates'].append(predicateText)
	index += 1
index = 0
for box in data['results'][1]['boxes']:
	boxText = "property(box" + str(index) + ", " + str(data['results'][1]['boxes'][index][0]) + ", " + str(data['results'][1]['boxes'][index][1]) + ", " + str(data['results'][1]['boxes'][index][2]) + ", " + str(data['results'][1]['boxes'][index][3]) + ")"
	outputData['boxes'].append(boxText)
	index += 1
denseInput.close()
with open('predicates0.json', 'w') as outfile:
